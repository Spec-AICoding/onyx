...
export const SWR_KEYS = {
  ...
  // ── External User Groups (Craft 外部用户组，独立 API 经 rewrites 代理) ──
  // 基础端点不带查询串：分页参数由 usePaginatedFetch 拼接，cc_pair_id 经 filter 传入。
  externalUserGroups: "/api/manage/admin/external-user-groups",
  externalUserGroupsProbe: (ccPairId: number) =>
    `/api/manage/admin/external-user-groups?cc_pair_id=${ccPairId}&page_num=0&page_size=1`,
  externalUserGroupMembers: (ccPairId: number, groupId: string) =>
    `/api/manage/admin/external-user-groups/members?cc_pair_id=${ccPairId}&group_id=${encodeURIComponent(groupId)}`,
  externalUserGroupsPublic: (ccPairId: number) =>
    `/api/manage/admin/external-user-groups/public?cc_pair_id=${ccPairId}`,

  // ── Sync Attempts（数据 → 同步文件列表的任务层） ────────────────────
  // 任务 = index_attempt；点任务后经 syncFiles?index_attempt_id= 下钻批次文件。
  ...
  // 由 next.config.js 的 /api/manage/admin/sync-files/:path* rewrite 透传。
  syncFiles: "/api/manage/admin/sync-files",
  syncFilesProbe: (ccPairId: number) =>
    `/api/manage/admin/sync-files?cc_pair_id=${ccPairId}&page_num=0&page_size=1`,
  syncFileDocuments: (fileId: string) =>
    `/api/manage/admin/sync-files/${fileId}/documents`,
  syncFileDownload: (fileId: string) =>
    `/api/manage/admin/sync-files/${fileId}/download`,
