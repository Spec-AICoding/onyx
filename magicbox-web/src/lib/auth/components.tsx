...
import { SvgLogOut, SvgCheckCircle, SvgXCircle, SvgSimpleLoader, SvgArrowRightCircle } from "@opal/icons";
...
const SESSION_END_COPY: Record<SessionEndReason, string> = {
  ...
  [SessionEndReason.TERMINATED]: "该会话已退出，请重新登录以继续。",
      ...
      {sessionEnded && (
        <Modal open>
          <Modal.Content width="sm" height="sm">
            <Modal.Header icon={SvgLogOut} title="您已退出登录" />
            <Modal.Body>
...
export function PasswordRequirements({ password }: PasswordRequirementsProps) {
  ...
  const rules = (
    [
      {
        label: `${passwordMinLength}–${passwordMaxLength} 个字符`,
  ...
  const validationSchema = useMemo(() => {
    ...
    if (isSignup) {
      ...
      passwordSchema = passwordSchema.test(
        "length",
        `密码长度须为 ${minLength}–${maxLength} 个字符`,
  ...
  async function handleSubmit(values: FormValues) {
    ...
    if (loginResponse.ok) {
      ...
      if (loginResponse.status === 429) {
        ...
        errorMsg = "请先注册账号以设置密码";
      } else if (errorDetail === "PASSWORD_LOGIN_DISABLED") {
        errorMsg = "密码登录已关闭，请使用您的身份提供商登录。";
      ...
      {({ isSubmitting, isValid, dirty, values, errors }) => {
        return (
          <AuthLayouts.FormBody>
            <AuthLayouts.Fields>
              <InputVertical title="邮箱地址" withLabel="email">
                <InputTypeInField
                  name="email"
              ...
              <div className="flex flex-col gap-1">
                <InputVertical
                  ...
                  topRight={
                    ...
                    !!values.email
                      ? markdown(
                          `[忘记密码？](/auth/forgot-password?email=${encodeURIComponent(values.email)})`
                        )
                      : undefined
                  }
                  subDescription={isSignup ? "密码要求：" : undefined}
                >
            ...
            <Button
              ...
              disabled={
                ...
                !(isValid && (!isSignup || Boolean(authTypeMetadata))) ||
                !dirty
              }
