...
export async function requireAdminAuth(): Promise<AuthCheckResult> {
  ...
  // Reaching the admin panel means holding some permission an admin route
  // requires — a scoped group manager may be a plain BASIC user, so a role
  // check would bounce them.
