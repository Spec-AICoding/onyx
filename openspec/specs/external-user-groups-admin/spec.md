# external-user-groups-admin Specification
...
magicbox-web SHALL 在管理后台"权限"分区下新增"外部用户组"菜单项（与"用户"、"用户组"并列，不依赖 `onyx_craft_available` 开关），路由为 `/admin/craft/external-groups`。页面 SHALL 提供两个 Tab：Tab1"用户-组映射"（连接器下拉 + 组聚合表格 + 行展开查看成员邮箱），Tab2"公开外部组"（连接器下拉 + 公开组列表）。页面 SHALL NOT 修改现有"权限/应用/指令"菜单及其页面。Tab2 无数据时 SHALL 展示引导文案说明仅 Google Drive 会写入公开外部组。

#### Scenario: 菜单可见性
- **WHEN** 管理员登录
- **THEN** 侧边栏"权限"分区下可见"外部用户组"菜单项（与"用户"、"用户组"并列），点击进入 `/admin/craft/external-groups` 页面
